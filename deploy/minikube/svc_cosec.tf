...
resource kubernetes_secret_v1 cosec {
  ...
  data = {
    ...
    POSTGRES_PASSWORD: random_password.cosec.result
  }
  depends_on = [
...
resource kubernetes_config_map_v1 cosec {
  metadata {
    name      = "cosec-config-map"
    namespace = var.project
  }
  ...
  data = {
      ...
      POSTGRES_PORT= "5432"
      POSTGRES_USER= "cosec_user"
      POSTGRES_DB_NAME= "cosec"
      POSTGRES_SEARCH_PATH= "cosec,public"
      HTTP_SERVER_PORT= "80"
...
resource kubernetes_deployment_v1 cosec {
  ...
  spec {
    ...
    template {
      ...
      spec {
        ...
        container {
          name              = "cosec"
          image             = "${var.image_repo}/cosec:latest"
  ...
  depends_on = [
    kubernetes_namespace_v1.namespace,
    kubernetes_config_map_v1.cosec,
