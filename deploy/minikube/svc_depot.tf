...
resource kubernetes_secret_v1 depot {
  ...
  data = {
    ...
    POSTGRES_PASSWORD: random_password.depot.result
  }
  depends_on = [
...
resource kubernetes_config_map_v1 depot {
  metadata {
    name      = "depot-config-map"
    namespace = var.project
  }
...
resource kubernetes_deployment_v1 depot {
  ...
  spec {
    ...
    template {
      ...
      spec {
        ...
        container {
          name              = "depot"
          image             = "${var.image_repo}/depot:latest"
  ...
  depends_on = [
    kubernetes_namespace_v1.namespace,
    kubernetes_config_map_v1.depot,
