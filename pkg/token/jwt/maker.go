...
func (maker *JWTMaker) CreateToken(username, userID string, duration time.Duration) (string, *token.Payload, error) {
	payload, err := token.NewPayload(username, userID, duration)
	if err != nil {
		return "", payload, err
	}
